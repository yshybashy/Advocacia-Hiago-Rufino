...
initEvents(){
    ...
    function getArtigo(){
        ...
        //FIM MOSTRA ARTIGOS RECENTES 
        let n=0;
        //MOSTRA ARTIGO PRINCIPAL 
        firebase.firestore().collection('Blog').doc('Info').collection('Artigos').doc(localStorage.blog).get().then(docx=>{

            let artigo = docx.data();
           // console.log(artigo);
                let div = document.createElement('div');
                div.innerHTML = `
                
                <p>
                <img  class="hero-wrap hero-wrap-2" src="${artigo.foto}" alt="" class="img-fluid">
              </p>
              <h2 class="mb-3">${artigo.titulo}</h2>
